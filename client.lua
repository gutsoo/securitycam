...
-- Events
RegisterNetEvent('securitycam:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        ESX.ShowNotification('No camera!')
    end
end)

RegisterNetEvent('securitycam:client:DisableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('securitycam:client:EnableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('securitycam:client:SetCamera', function(key, isOnline)
